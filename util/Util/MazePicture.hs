...
mazeToPicture :: Int -> Maze a -> (Int -> Int -> a -> Picture) -> (Int, Int, Picture)
mazeToPicture cellSize maze drawCell =
    ...
    where  (Maze cells) = maze
           ...
           locatedCell (Located (Location x y) c) = translate (fromIntegral $ x * cellSize) (fromIntegral $ y * cellSize) $ drawCell x y c
           pic = translate 
